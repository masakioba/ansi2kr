...
int str2ncmp(char *s1,char *s2,int n)
{
  int i;
  for(i=0;i<n;++i){
    if(*s1 != *s2){
...
int srch_a(char *s,char *u,int *lenm)
{
  int i,j,len,start,awanai;
  i = 0;
...
s/src/dst/g
  ...
  adefbbbdefccc
*/
void repl_a_all(char *str,char *m,char *g)
{
  int a,mlen,slen;
  int glen;
  char go[MAXBLEN+50];
...
s/\<src\>/dst/g
*/
int srch_tango_a(char *s,char *u,int *lenm)
{
  int i,j,len,start,awanai;
  int a,kkmf,kkgf;
...
s/\<src\>/dst/g
  ...
  adefbbb def ccc
*/
void repl_ta_all(char *str,char *m,char *g)
{
  int a,mlen,slen;
  int glen;
  char go[MAXBLEN+50];
...
void chg_kata(char *str,int mode,int voidf)
{
  /* for BDS C and MSX-C */
  if(voidf){
    repl_ta_all(str,"void","");
...
int main(int argc,char **argv)
{
  ...
  if(NULL == (fp = fopen(infile,"rt"))){
    fprintf(stderr,"Error: input file can't open[%s]%c\n",infile,7);
    return -1;
  }
  if(Outf){
    if(NULL == (Outp = fopen(outfile,"wt"))){
  ...
  while(1){
    buf[0] = 0;
    if(NULL == fgets(buf,WORDLENMAX,fp)){
      fer = 1;
      if(buf[0] == 0){
        break;
      }
